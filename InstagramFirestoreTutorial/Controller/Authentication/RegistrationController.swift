...
class RegistrationController: UIViewController {
    ...
    private let alreadyHaveAccountButton: UIButton = {
        let button = UIButton(type: .system)
        button.attributedTitle(firstPart: "Already have an account?", secontPart: "Login")
    ...
    @objc func handleSignUp() {
        ...
        AuthService.registerUser(withCredential: credentials) { error in
            if let error = error {
                print("DEBUG: Faield to register user \(error.localizedDescription)")
                return
            }
    ...
    func configureUI() {
        ...
        stack.anchor(top: plushPhotoButton.bottomAnchor, left: view.leftAnchor, right: view.rightAnchor, paddingTop: 32,
